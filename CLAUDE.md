...
uv run python server.py       # FastAPI 服务（端口 8000）
...
- `Agent`（`src/agent/agent.py`）：封装 LLM + tools，`chat()` 单轮，`chat_stream()` SSE 流式输出
- 工具通过 `@tool` 装饰器定义，注册到 `agent.py` 的 `tools` 列表
- `DocumentPipeline`（`src/rag/pipeline.py`）：文档处理管线（加载→分块→嵌入→存储）
- `VectorStore`（`src/rag/vectorstore.py`）：ChromaDB 封装，支持 CRUD 和相似度搜索
- `knowledge_search` 工具（`src/tools/knowledge.py`）：延迟初始化管线，Agent 按需调用
...
- 中文注释和文档字符串
- `src/` 下按功能分包（config、agent、tools、rag），每个包有 `__init__.py` 导出公共接口
- 配置集中在 `Settings` dataclass（`src/config/settings.py`），环境变量优先于默认值
