...
- **入库**：前端 → `/api/knowledge/upload` → `DocumentPipeline` → 加载 → 微信清洗 → 中文分块 → SHA256 去重短路 → Chroma Cloud 嵌入入库
...
| `DELETE` | `/api/knowledge/{filename}` | 删除指定文档（向量库 + 本地文件） |
| `POST` | `/api/knowledge/search` | 直接检索（调试用） |
| `GET` | `/api/health` | 健康检查 |
...
- **前端**：独立项目仓库（React + Vercel AI SDK），通过 SSE 调用 `/api/chat`，知识库管理调用 `/api/knowledge/*`
